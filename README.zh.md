...
<details>
<summary><strong>性能基准测试</strong> — 在 Apple Silicon M4 Pro 上测试了 8 个模型</summary>

### 基准测试

30 个会话，67K 词输入：

| 模型 | 参数量 | 平均/会话 | 总计（30 会话） | 压缩比 | 错误 | 备注 |
|---|---|---|---|---|---|---|
| `qwen3:14b` | 14B | ~36s | ~18 分钟 | 15:1 | 0 | 简洁、结构化。**默认** |
| `glm4:9b` | 9B | ~20s | ~10 分钟 | 18:1 | 0 | 快速，高压缩 |
| `mistral-nemo` | 12B | ~27s | ~14 分钟 | 14:1 | 0 | 稳定全能 |
| `gemma3:12b` | 12B | ~34s | ~17 分钟 | 10:1 | 0 | 输出更详细 |
| `qwen3.5:9b` | 9B | ~32s | ~16 分钟 | 10:1 | 0 | 相对于其大小速度偏慢 |
| `granite3.3:8b` | 8B | ~26s | ~13 分钟 | 11:1 | 0 | 速度和质量的良好平衡 |
| `phi4-mini` | 3.8B | ~10s | ~5 分钟 | 12:1 | 0 | 最快，但事实准确性较低 |
| `nemotron-mini` | 4.2B | ~4s | ~2 分钟 | 550:1 | 8/30 | 不推荐 — 频繁出现空响应 |

**质量对比**（3 个会话并排比较）：
- `qwen3:14b`：最佳事实准确性，结构化要点，尊重对话语言
- `phi4-mini`：快 3.4 倍但引入事实错误（如在某会话中颠倒了一个决策）
- `granite3.3:8b`：速度和质量之间的良好折中

**推荐**：追求准确性用 `qwen3:14b`，需要更快速度且不牺牲可靠性用 `glm4:9b` 或 `granite3.3:8b`。

- 嵌入（`nomic-embed-text`）：30 个会话仅需数秒
- 语义搜索：即时响应（每次查询 <1s）
- 会话压缩：智能 head/tail 策略 — 保留开头和结尾的上下文，压缩中间部分。可处理 1K 到 1M+ 字符的会话
