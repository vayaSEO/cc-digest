...
<details>
<summary><strong>パフォーマンスベンチマーク</strong> — Apple Silicon M4 Pro で 8 モデルをテスト</summary>

### ベンチマーク

30 セッション、67K ワード入力：

| モデル | パラメータ | 平均/セッション | 合計（30 セッション） | 圧縮率 | エラー | 備考 |
|---|---|---|---|---|---|---|
| `qwen3:14b` | 14B | ~36s | ~18 分 | 15:1 | 0 | 簡潔で構造化。**デフォルト** |
| `glm4:9b` | 9B | ~20s | ~10 分 | 18:1 | 0 | 高速、高圧縮 |
| `mistral-nemo` | 12B | ~27s | ~14 分 | 14:1 | 0 | 安定したオールラウンダー |
| `gemma3:12b` | 12B | ~34s | ~17 分 | 10:1 | 0 | より詳細な出力 |
| `qwen3.5:9b` | 9B | ~32s | ~16 分 | 10:1 | 0 | サイズの割に遅い |
| `granite3.3:8b` | 8B | ~26s | ~13 分 | 11:1 | 0 | 速度と品質の良いバランス |
| `phi4-mini` | 3.8B | ~10s | ~5 分 | 12:1 | 0 | 最速だが事実精度が低い |
| `nemotron-mini` | 4.2B | ~4s | ~2 分 | 550:1 | 8/30 | 非推奨 — 空レスポンスが頻発 |

**品質比較**（3 セッション並列比較）：
- `qwen3:14b`：最高の事実精度、構造化された箇条書き、会話言語を尊重
- `phi4-mini`：3.4 倍速いが事実誤りを導入（あるセッションで決定を逆にした等）
- `granite3.3:8b`：速度と品質の間の堅実な中間点

**推奨**：精度重視なら `qwen3:14b`、信頼性を犠牲にせず高速化するなら `glm4:9b` または `granite3.3:8b`。

- 埋め込み（`nomic-embed-text`）：30 セッションを数秒で処理
- セマンティック検索：即時応答（クエリあたり <1s）
- セッション圧縮：スマートな head/tail 戦略 — 冒頭と末尾のコンテキストを保持し、中間を圧縮。1K ～ 1M+ 文字のセッションに対応
